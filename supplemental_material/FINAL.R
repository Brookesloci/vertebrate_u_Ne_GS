...
groups_to_model <- c("Bird", "Mammal", "Fish", "Reptile")

reg_lines <- data.frame()
results_table <- data.frame()

# Run PGLS regression on all taxa combined
comp_data_all <- comparative.data(phy = time_tree,
                                  data = d1,
...
model_all <- pgls(genome_size ~ Ne..harmonic.mean..psmc., data = comp_data_all, lambda = "ML")

# Extract coefficients for the combined PGLS regression
coef_all <- coefficients(model_all)
intercept_all <- coef_all[1]
slope_all <- coef_all[2]
lambda_all <- model_all$param["lambda"]
p_value_all <- summary(model_all)$coefficients[2, "Pr(>|t|)"]

# Add combined PGLS results to the table
results_table <- rbind(results_table, data.frame(
  Taxonomic.group = "All Taxa",
  Effect.Size = slope_all,
  Estimated.Lambda = lambda_all,
  P.Value = p_value_all
))

# Loop through each taxonomic group
for (taxon in groups_to_model) {
  
  # Subset data
  sub_data <- d1 %>% filter(Taxonomic.group == taxon)
  
  # Prune the tree to match the species in the subset
  pruned_tree <- drop.tip(time_tree, setdiff(time_tree$tip.label, sub_data$Species.name))
  
  # Create comparative data object
  comp_data <- comparative.data(phy = pruned_tree,
                                data = sub_data,
                                names.col = "Species.name",
                                vcv = TRUE,
                                na.omit = TRUE,
  ...
  # Run PGLS with ML estimate of lambda
  model <- pgls(genome_size ~ Ne..harmonic.mean..psmc., data = comp_data, lambda = "ML")
  
  # Extract coefficients
  coef <- coefficients(model)
  
  # Extract effect size, lambda, and p-value
  effect_size <- coef[2]  # Slope represents effect size
  lambda_est <- model$param["lambda"]
  p_value <- summary(model)$coefficients[2, "Pr(>|t|)"]
  
  # Store in regression results table
  results_table <- rbind(results_table, data.frame(
  ...
  # Store PGLS regression line values
  reg_lines <- rbind(reg_lines, data.frame(
    Taxonomic.group = taxon,
    intercept = coef[1],
    slope = coef[2]
  ))
...
# Print results table with combined PGLS regression
cat("\nPGLS Regression Results (Including All Taxa):\n")
print(results_table)

# Plot all points with PGLS regression lines
r_all <- ggplot(d1, aes(x = Ne..harmonic.mean..psmc., y = genome_size, color = Taxonomic.group)) +
  geom_point() +
  scale_color_manual(values = my_colors) +
  xlab("Harmonic mean Ne (log10)") +
  ylab("Genome size (log10 bp)") +
  theme(
...
# Add PGLS regression lines
for (i in 1:nrow(reg_lines)) {
  r_all <- r_all +
    geom_abline(
      intercept = reg_lines$intercept[i],
      slope = reg_lines$slope[i],
      color = my_colors[[reg_lines$Taxonomic.group[i]]],
      linetype = "solid"
    )
}

# Add PGLS regression line for all taxa in gray
r_all <- r_all +
  geom_abline(intercept = intercept_all, slope = slope_all, color = "gray", linetype = "solid")

# Print plot
r_all

d2 <- d1[d1$Taxonomic.group == "Fish",]

d2$Salinity <- c(3,3,2,3,1,2,2,1)

tree_fish <- time_tree
fish_tips <- c("Amphiprion_ocellaris" ,  "Larimichthys_crocea"   , "Cynoglossus_semilaevis", "Paralichthys_olivaceus","Betta_splendens"       ,"Syngnathus_scovelli"    ,"Salmo_salar"     ,       "Cyprinus_carpio"   )
tree_fish <- keep.tip(tree_fish,fish_tips)

d2 <-ReorderData(tree_fish,d2)

name.check(tree_fish,d2)

comp_data_fish <- comparative.data(phy = tree_fish,
                                   data = d2,
                                   names.col = "Species.name",
                                   vcv = TRUE,
                                   na.omit = TRUE,
...
pgls_fish <- pgls(genome_size ~ Salinity, data = comp_data_fish, lambda = "ML")


coef_fish <- coefficients(pgls_fish)
intercept_fish <- coef_fish[1]
slope_fish <- coef_fish[2]
lambda_fish <- pgls_fish$param["lambda"]
p_value_fish <- summary(pgls_fish)$coefficients[2, "Pr(>|t|)"]


summary(pgls_fish)

ols_fish <- lm(genome_size~ Salinity, data=d2)
summary(ols_fish)

salinity_labels <- c("1" = "Freshwater", "2" = "Euryhaline", "3" = "Marine")
d2$Salinity <- factor(d2$Salinity, levels = c(1, 2, 3), labels = c("Freshwater", "Euryhaline", "Marine"))

salinity_colors <- c("Freshwater" = "#1f77b4", "Euryhaline" = "#ff7f0e", "Marine" = "#2ca02c")

plot_fish <- ggplot(d2, aes(x = Salinity, y = genome_size, color = factor(Salinity))) +
  geom_point(size = 3) + 
  geom_abline(intercept = intercept_fish, slope = slope_fish, linetype = "solid", color = "red") +
  scale_color_manual(values = salinity_colors, labels = salinity_labels) +
  geom_text(aes(label = Species.name), hjust = -0.2, vjust = -0.5, size = 3.5) +
  labs(x = "Salinity Level", y = "Genome Size (log10 bp)", color = "Salinity") +
  theme_minimal() +
...
##show LP
d7_vars <- c("Species.name","Taxonomic.group","diversity" ,"Nc", "expected_pi")
d7 <- d[d7_vars]

d7 <- d7 %>% drop_na(Nc)
d7 <- d7 %>% drop_na(expected_pi)
d7 <- d7 %>% drop_na(diversity)

tip5 <- c("Arctocephalus_gazella","Larus_argentatus", "Tupaia_belangeri" ,"Panthera_tigris" , "Paralichthys_olivaceus","Rangifer_tarandus")
...
pagel_pi_Nc <- gls(diversity ~Nc, data=d7,correlation=corPagel(value=0.8,phy=time_tree_Nc_u,fixed=FALSE))

Pagel0_pi_Nc <- gls(diversity ~Nc, correlation = corPagel(value=0, phy=time_tree_Nc_u, fixed = TRUE), data=d7)

anova(pagel_pi_Nc,Pagel0_pi_Nc)

summary(pagel_pi_Nc)

pi_Nc.lm <- lm(diversity ~Nc, data = d7)

summary(pi_Nc.lm)

vert7 <- comparative.data(phy=time_tree_Nc_u, data=d7, names.col = Species.name, vcv = TRUE, na.omit = TRUE, warn.dropped = TRUE)

pi_Nc_pgls <- pgls(diversity ~Nc, data=vert7)
summary(pi_Nc_pgls)
...
r6 <- ggplot(data = d7, aes(x = Nc, y = diversity)) +
